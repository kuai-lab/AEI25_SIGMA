...
def render_set(args, load2gpt_on_the_fly, name, iteration, views, gaussians, pipeline, background, deform, eval_app=False, visualize=False):
    ...
    try:
        ...
        results_list, real_perm = eval_axis_and_state_all(pred_joint_list, pred_joint_types, gt_info_list, print_perm=True)
        print(results_list)
    except:
    ...
    for mask_id in range(-1, num_d_joints + 1):
        ...
        for idx, view in enumerate(tqdm(views)):
            ...
            if view.fid < 0.5:
                ...
                # depths_end.append(view.depth.cuda()[None])
...
def render_sets(args, dataset: ModelParams, iteration, pipeline: PipelineParams, skip_train: bool, skip_test: bool, mode: str, load2device_on_the_fly=False):
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(args, load2device_on_the_fly, "train", scene.loaded_iter, cam_traj, gaussians, pipeline, background, deform, eval_app=args.eval_app, visualize=args.visualize)

        if not skip_test:
            render_set(args, load2device_on_the_fly, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, deform, eval_app=args.eval_app, visualize=args.visualize)
